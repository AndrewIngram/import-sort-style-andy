...
export default function(styleApi) {
  ...
  const isArchitectureModule = (imported) =>
    Boolean(imported.moduleName.match(/^(next|@emotion|date-fns)/));
  const isFirstPartyModule = (imported) =>
    Boolean(imported.moduleName.match(/^(~)/));
  const isFirstPartyComponentModule = (imported) =>
    Boolean(imported.moduleName.match(/^(~\/components)/));
  ...
  return [
    ...
    // import { foo } from "~/util.js";
    {
      match: and(
        isFirstPartyModule,
        not(isFirstPartyComponentModule),
        not(isStylesModule),
        not(isImageModule)
      ),
      sort: moduleName(naturally),
    ...
    {
      match: isFirstPartyComponentModule,
      sort: moduleName(naturally),
      sortNamedMembers: alias(unicode),
    },
    { separator: true },
    ...
    {
      match: and(isFirstPartyModule, isImageModule),
      sort: [dotSegmentCount, moduleName(naturally)],
      sortNamedMembers: alias(unicode),
    },
    { separator: true },
